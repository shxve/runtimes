...
<p>This command retrieves information on how to fund the dependencies of a given project.
...
If a package name is provided then it tries to open its funding url using the <a href="../using-npm/config#browser.html"><code>--browser</code> config</a> param; if there are multiple funding sources for the package, the user will be instructed to pass the
<code>--which</code> option to disambiguate.</p>
<p>The list will avoid duplicated entries and will stack all packages that share the same url as a single entry.
Thus, the list does not have the same shape of the output from <code>npm ls</code>.</p>
<h4 id="example">Example</h4>
<h3 id="workspaces-support">Workspaces support</h3>
<p>It's possible to filter the results to only include a single workspace and its dependencies using the <a href="../using-npm/config#workspace.html"><code>workspace</code> config</a> option.</p>
<h4 id="example2">Example:</h4>
<p>Here's an example running <code>npm fund</code> in a project with a configured workspace <code>a</code>:</p>
...
<p>And here is an example of the expected result when filtering only by a specific workspace <code>a</code> in the same project:</p>
